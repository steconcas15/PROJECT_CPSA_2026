...
class DrowsinessClassifier:
    ...
    def _process_state_machine(self, theta: float, omega: float, t_now: float) -> int:
        ...
        # ── EVENTO 2: COLPO DI SONNO / OSCILLAZIONE (NOD) ─────────────────────
                ...
                if 0.20 <= elapsed_time <= 3.0:
                    ...
                    if deltas_slice:
                        ...
                        if abs(peak_delta) > self.nod_angle_min:
                            log_system(f"🚨 [NOD] Colpo di sonno / ammicco rilevato! t={t_now:.2f}s, Δθ_picco={peak_delta:.1f}°", level="WARNING")
                            self.last_event_time = t_now
                            self.in_nod = False
                            return 2  # Allineato a LABELS[2] = "NOD"
                self.in_nod = False
