...
class EventDispatcher:
    ...
    def _process_policy_for_event(self, event, tag, now_time):
        ...
        # --- FILTRO BLINK PER LO SPEAKER ---
        # Se la predizione è DROWSY, passiamo lo stato alla policy solo se 
        # persiste in modo continuativo da almeno 1.0 secondo.
        if current_pred == "DROWSY":
            if self._drowsy_since_ts is None or (now_time - self._drowsy_since_ts) < 1.0:
                current_pred = None  # Sovrascriviamo a None per bloccare l'allarme dello speaker
